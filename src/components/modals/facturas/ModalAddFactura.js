...
class ModalAddFactura extends React.Component {
    ...
    peticionPost = async (event) => {
        ...
        let auxArray = this.state.arrayFacturaLibros;
        //iteramos el auxiliar del array de libros de la Factura
        ...
        axios(config)
            .then((response) => {
                ...
                if (response.data.succes) {
                    swal("Factura creada correctamente", {
                        icon: "success",
                    }).then(() => {
                        this.props.onCloseModal();
                        this.props.fetchDataFacturas();
    ...
    cleanModal = async () => {
        await this.setState({ form: {}, arrayFacturaLibros: null });
        this.props.onCloseModal();
    }
    ...
    render() {
        ...
        const popLibro = async (index) => {
            let auxArray = this.state.arrayFacturaLibros; //declaramos un arreglo auxiliar
            ...
            auxArray.length === 0 ?
                await this.setState({
                    ...this.state,
                    arrayFacturaLibros: null
                }) :
                await this.setState({
                    ...this.state,
                    arrayFacturaLibros: auxArray
                });
        ...
        const pushLibro = async (libro) => {
            ...
            if (this.state.arrayFacturaLibros) { //preguntamos si esta vacio el arreglo de libros - Factura

                let noExiste = true;
                //for para iterar sobre el array que tenemos y comprobar que no tenemos mas de un libro repetido en el array
                for (let i = 0; i < this.state.arrayFacturaLibros.length; i++) {
                    //validamos que si existe el mismo libro aumente 1 en su contador 
                    if (this.state.arrayFacturaLibros[i].idLibro === libro.idLibro) {
                                ...
                                <Col xs={12} md={3}>
                                    ...
                                    <Col md={12} className="mt-3">
                                        <div className="d-grid gap-2">
                                            {
                                                this.state.arrayFacturaLibros ?
                                                    this.state.arrayFacturaLibros.map((libro, index) => {
                                                        return (
                                                    ...
                                                    <Alert variant="warning"><p>No hay libros en esta factura, seleccione los libros que desea agregar a la factura de la tabla siguiente.</p></Alert>
                                            }
                                        </div>
                                    </Col>
                                </Col>
...
export default ModalAddFactura;